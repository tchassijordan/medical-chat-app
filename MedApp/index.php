    ...
    <body>
        <header>
            <nav class="nav-bar-container">
                ...
                <ul class="nav-bar">
                    <li>
                        <a href="./MedApp/chat/index.php" class="nav-item"> <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="message" class="svg-inline--fa fa-message" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                    ...
                    <li class="active">
                        <a href="./MedApp/index.php" class="nav-item"><svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="user-group" class="svg-inline--fa fa-user-group" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
        ...
        <main>
            <div class="container">
                ...
                ?>
					<div class="in-categorie flex-col">
						<h1 class="title"><?php echo $_GET['categorie']; ?></h1>
						<a href="addSujet.php?categorie=<?php echo $_GET['categorie']; ?>" class="add-cat-subject">Ajouter un sujet <span class="left-arrow">&#8594;</span></a>
						<h3 class="title-prev">Sujet Precedent</h3>
						<?php
						$requete = $bdd->prepare('SELECT * FROM sujet WHERE categorie = :categorie ');
						$requete->execute(array('categorie' => $_GET['categorie']));
						?>
						<div class="topics-container flex-row">
							<?php
							while ($reponse = $requete->fetch()) {
							?>
								<a href="index.php?sujet=<?php echo $reponse['name'] ?>" class="topic cat-topic"><?php echo $reponse['name'] ?> <span class="left-arrow">&#8594;</span></a>
							<?php
							}
							?>
						</div>
                    ...
                    ?>
                    <div class="categories">
                        <h1 class="title"><?php echo $_GET['sujet']; ?></h1>
                    </div>
                    <?php
                    ...
                    <div class="messages-container">
                        <div class="messages-area flex-col">
                            <?php
                            while ($reponse = $requete->fetch()) {
                            ?>
                                <div class="msg-doc-container flex-col">
                                    <?php
                                    $requete2 = $bdd->prepare('SELECT * FROM membres WHERE id = :id');
                                    $requete2->execute(array('id' => $reponse['propri']));
                                    $membres = $requete2->fetch();
                                    ?>
                                    <p class="msg msg-doc msg-txt"><?php echo $reponse['contenu']; ?></p>
                                    <p class="msg-date"><?php echo $membres['pseudo']; ?></p>
                                    <?php
                                    ?>
                                </div>
                            <?php
                        ...
                        <form method="post" action="index.php?sujet=<?php echo $_GET['sujet']; ?>" class="msg-form">
                            <textarea class="input-msg answer-msg" name="sujet" id="answer-msg" cols="30" rows="2" placeholder="Enter your message here...."></textarea>
                            <input type="hidden" name="name" value="<?php echo $_GET['sujet']; ?>" />
                            <button type="submit" class="btn send-msg-btn bck-btn">Ajouter à la conversation</button>
                            <?php
                            if (isset($erreur)) {
                                echo $erreur;
                            }
